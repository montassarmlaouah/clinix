...
import { ALERTES, PATIENTS } from '@/src/api/endpoints';
...
export default function AlertesScreen(): React.JSX.Element {
  ...
  // ── Chargement patients pour recherche locale ─────────────────────────────
  ...
  // ── Recherche patient (filtre local, debounce 400ms) ─────────────────────
  ...
  function playSuccess(anim: Animated.Value, reset: () => void) {
    Animated.spring(anim, {
      toValue: 1, useNativeDriver: true, friction: 5, tension: 120,
    }).start();
    setTimeout(() => {
  ...
  // CORRECTION : le backend POST /api/alertes/urgence attend uniquement { message: string }
  async function handleSubmitMedecin() {
    ...
    setErrorMed('');
    try {
      const niveauLabel = NIVEAU_CONFIG[niveau].label;
      await apiPost(ALERTES.URGENCE, {
        message: `[${niveauLabel}] Patient: ${selectedPatient.prenom} ${selectedPatient.nom} — ${descMedecin.trim()}`,
      });
  ...
  // CORRECTION : le backend POST /api/alertes/manque-materiel attend { description: string }
  async function handleSubmitMateriel() {
    ...
    try {
      await apiPost(ALERTES.MATERIEL, {
        description: `[${service.trim()}] ${descMateriel.trim()}`,
      });
...
const styles = StyleSheet.create({
  ...
  // Formulaire
  formContent: { padding: spacing.xxl, paddingBottom: 80 },
  fieldLabel:  {
